...
def prepare_data(df):
    ...
    X = df[['hour', 'temp_c', 'precip_mm', 'wind_kph', 'humidity', 'pressure_mb', 'temp_variation', 'pressure_variation', 'humidity_variation', 'wind_humidity_interaction']]
    y = df[['temp_c', 'precip_mm', 'wind_kph', 'humidity', 'pressure_mb', 'vis_km']]
    
    return X, y

# Oti du mod rdn forest
...
def predict_next_hour(model, last_row):
    ...
    X_next = next_hour_data[['hour', 'temp_c', 'precip_mm', 'wind_kph', 'humidity', 'pressure_mb', 'temp_variation', 'pressure_variation', 'humidity_variation', 'wind_humidity_interaction']].values.reshape(1, -1)
...
# prediction
def plot_predictions(y_test, y_pred, target_variable, variable_name):
    plt.figure(figsize=(10, 6))
    plt.plot(y_test[target_variable].values, label=f'Actual {variable_name}', color='blue')
    plt.plot(y_pred[:, target_variable], label=f'Predicted {variable_name}', color='red', linestyle='--')
...
def main():
    df = load_data('api_meteot\historical_data.csv')
    ...
    last_row = X.iloc[-1]
    next_hour_data = predict_next_hour(xgb_model, df.iloc[-1])
